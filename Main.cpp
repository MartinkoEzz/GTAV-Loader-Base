...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, g_Name.c_str(), NULL};
    RegisterClassEx(&wc);
    g_MainHWND = CreateWindow(wc.lpszClassName, g_Name.c_str(), WS_POPUP, 0, 0, 5, 5, NULL, NULL, wc.hInstance, NULL);

    // Initialize Direct3D
    if (!CreateDeviceD3D(g_MainHWND))
    {
        CleanupDeviceD3D();
    ...
    // Check if loader is killswitched
    if (Utility::Killswitch())
        exit(0);

    // Download Files
    Utility::DownloadFiles();
    std::thread t1(Utility::DownloadDll);
    std::string key = "";
    // Show the window
    ...
    io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\seguiemj.ttf", 18);
    Interface::bigFont = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\seguiemj.ttf", 22);
    ...
    ImGui_ImplWin32_Init(g_MainHWND);
    ...
    while (Message.message != WM_QUIT)
    {
        ...
        g_Device->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0, 1.0f, 0);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE:
        ...
        {
            g_Parameters.BackBufferWidth = LOWORD(lParam);
            g_Parameters.BackBufferHeight = HIWORD(lParam);
            ResetDevice();
        }
        return 0;
    case WM_SYSCOMMAND:
